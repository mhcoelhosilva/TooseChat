...
Blowfish::Blowfish(const std::string& inKey)
{
    m_key = inKey;

    // initialize m_P and m_S
    std::memcpy(m_P, initialP, sizeof(initialP));
    ...
    for (int i = 0; i < 18; i++) 
    {
        ...
        if (*p != 0 )
        {
            std::cerr << "Key must be hex number" << std::endl;
            return;
        }
...
{
   uint32_t h = m_S[0][x >> 24] + m_S[1][x >> 16 & 0xff];
   return (h ^ m_S[2][x >> 8 & 0xff]) + m_S[3][x & 0xff];
}

void Blowfish::blowfishEncrypt(uint32_t *L, uint32_t *R) 
{
...
{
    if (outDst != inSrc)
        memcpy(outDst, inSrc, inByteLength);

    unsigned char* uc = (unsigned char*)outDst;

    for (int i = 0; i < inByteLength/sizeof(uint64_t); ++i)
    {
        uint32_t* left  = &reinterpret_cast<uint32_t*>(uc)[i * 2];
        uint32_t* right = &reinterpret_cast<uint32_t*>(uc)[i * 2 + 1];
        blowfishEncrypt(left, right);
    }
}

void Blowfish::decrypt(char* outDst, const char* inSrc, int inByteLength)
{
    if (outDst != inSrc)
        memcpy(outDst, inSrc, inByteLength);
